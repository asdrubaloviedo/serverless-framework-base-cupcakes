...
const RecipeController = require('@recipe/controller/recipe');
...
const normalize = (event) => {
  ...
  // 3) módulo recipes (ej: /recipes)
  const mod = ensureSlash(process.env.RECIPE_MODULE || 'recipes');
  p = strip(p, mod);
...
const routes = {
  'GET /receta': (e) => RecipeController.getById(qs(e))
};
