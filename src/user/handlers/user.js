require('module-alias/register');

const UserController = require('@user/controller/user');

const {
  validateCreateUserMedalLeage,
  validatePatchUserMedalLeage,
  validateCreateUserPackage,
  validateCreateUser,
  validateUpdateUser,
  validateGetUserPreferences,
  validateUpdateUserPreferences,
  validateSendPasswordResetEmail
} = require('@user/schema/user');


/*
 * =========================================================
 * RESPONSE HELPERS
 * =========================================================
 */

const ok = (body, code = 200) => ({
  statusCode: code,
  body: JSON.stringify(body)
});


const fail = (err) => {

  const code = Number(err?.statusCode) || 500;

  return {
    statusCode: code,
    body: JSON.stringify({
      message: err?.message || 'Error interno del servidor'
    })
  };
};


/*
 * Envuelve cada ejecución para centralizar el manejo
 * de respuestas correctas y errores.
 */
const withHandler = (fn) => async (event) => {

  try {

    return ok(
      await fn(event)
    );

  } catch (e) {

    console.error(e);

    return fail(e);
  }
};


/*
 * Obtiene el método HTTP tanto para API Gateway REST
 * como para HTTP API.
 */
const methodOf = (e) =>
  (
    e.httpMethod ||
    e.requestContext?.http?.method ||
    'GET'
  ).toUpperCase();


/*
 * =========================================================
 * PATH NORMALIZATION
 * =========================================================
 */

const ensureSlash = (s) =>
  (
    s
      ? (
          s.startsWith('/')
            ? s
            : `/${s}`
        )
      : ''
  );


const strip = (p, pref) => {

  if (!pref) {
    return p;
  }

  if (p === pref) {
    return '/';
  }

  if (p.startsWith(pref + '/')) {
    return p.slice(pref.length);
  }

  return p;
};


const normalize = (event) => {

  let p = event.rawPath || event.path || '/';


  /*
   * 1) Stage por ambiente.
   */
  const stage = event.requestContext?.stage
    ? `/${event.requestContext.stage}`
    : '';

  p = strip(p, stage);


  /*
   * 2) Endpoint root.
   *
   * Ejemplo:
   * /cupcakeslife
   */
  const root = ensureSlash(
    process.env.ENDPOINT_ROOT
  );

  p = strip(p, root);


  /*
   * 3) Módulo de usuarios.
   *
   * Ejemplo:
   * /usuarios
   */
  const mod = ensureSlash(
    process.env.USER_MODULE || 'usuarios'
  );

  p = strip(p, mod);


  /*
   * Normalización final.
   */
  if (!p.startsWith('/')) {
    p = `/${p}`;
  }

  if (
    p.length > 1 &&
    p.endsWith('/')
  ) {
    p = p.slice(0, -1);
  }

  return p;
};


/*
 * =========================================================
 * REQUEST HELPERS
 * =========================================================
 */

const qs = (e) =>
  e.queryStringParameters || {};


const json = (e) => {

  if (!e?.body) {
    return {};
  }

  try {

    return typeof e.body === 'string'
      ? JSON.parse(e.body)
      : e.body;

  } catch {

    throw {
      statusCode: 400,
      message: 'JSON inválido'
    };
  }
};


/*
 * Helper para el uso de validaciones con Zod.
 *
 * Además de validar, devuelve los datos ya saneados
 * por el schema correspondiente.
 */
const validBody = (e, validator) => {

  const data = json(e);

  const res = validator(data);


  if (!res.success) {

    const i = res.error.issues?.[0];

    const field = i?.path?.join('.');


    /*
     * Uniforma mensajes cuando un campo obligatorio
     * llega como undefined.
     */
    const msg =
      (
        i?.code === 'invalid_type' &&
        i?.received === 'undefined'
      )
        ? (
            field === 'email'
              ? 'User email is required.'
              : 'Field is required.'
          )
        : (
            i?.message ||
            'Parámetros inválidos'
          );


    throw {
      statusCode: 400,
      message: msg,
      field
    };
  }


  /*
   * Devolvemos los datos validados y saneados.
   */
  return res.data;
};


/*
 * =========================================================
 * ROUTES
 * =========================================================
 *
 * Todas las operaciones del módulo usuario se concentran
 * aquí y luego son resueltas por el mismo Lambda.
 */
const routes = {

  'POST /insertar-usuario-medalla':
    (e) =>
      UserController.createOneUserMedalLeage(
        validBody(
          e,
          validateCreateUserMedalLeage
        )
      ),


  'PATCH /actualizar-usuario-medalla':
    (e) =>
      UserController.patchOneUserMedalLeage(
        validBody(
          e,
          validatePatchUserMedalLeage
        )
      ),


  'POST /insertar-usuario-paquete':
    (e) =>
      UserController.createOneUserPackage(
        validBody(
          e,
          validateCreateUserPackage
        )
      ),


  'POST /insertar-usuario-nuevo':
    (e) =>
      UserController.createOneUser(
        validBody(
          e,
          validateCreateUser
        )
      ),


  /*
   * =========================================================
   * UPDATE USER
   * =========================================================
   *
   * Actualiza el nombre y el avatar del usuario.
   */
  'PATCH /actualizar-usuario':
    (e) =>
      UserController.updateOneUser(
        validBody(
          e,
          validateUpdateUser
        )
      ),


  /*
   * =========================================================
   * USER PREFERENCES
   * =========================================================
   */

  /*
   * Obtiene las preferencias guardadas del usuario.
   *
   * Recibimos el email en el body para mantener el mismo
   * patrón de validación utilizado por los demás endpoints
   * de usuario.
   */
  'POST /obtener-preferencias':
    (e) =>
      UserController.getUserPreferences(
        validBody(
          e,
          validateGetUserPreferences
        )
      ),


  /*
   * Actualiza las seis preferencias del usuario.
   */
  'PATCH /actualizar-preferencias':
    (e) =>
      UserController.updateUserPreferences(
        validBody(
          e,
          validateUpdateUserPreferences
        )
      ),


  /*
   * =========================================================
   * PASSWORD RESET
   * =========================================================
   *
   * Recibe únicamente el email del usuario.
   *
   * El controlador se encarga después de:
   * 1. Generar el enlace con Firebase.
   * 2. Enviar el correo personalizado.
   */
  'POST /recuperar-contrasena':
    (e) =>
      UserController.sendPasswordResetEmail(
        validBody(
          e,
          validateSendPasswordResetEmail
        )
      )
};


/*
 * =========================================================
 * MAIN HANDLER
 * =========================================================
 */

const handler = withHandler(
  async (event) => {

    /*
     * Ejemplo de key resultante:
     *
     * POST /recuperar-contrasena
     */
    const key =
      `${methodOf(event)} ${normalize(event)}`;


    const fn = routes[key];


    if (!fn) {

      return ok(
        {
          message: 'Not Found',
          pathTried: key
        },
        404
      );
    }


    return fn(event);
  }
);


module.exports = {
  handler
};