...
describe('CupcakeRepository filtros – ramas dinámicas', () => {
  ...
  test('getAllWithFilters: todos los filtros -> placeholders $2..$5 y params en orden', async () => {
    ...
    await r.getAllWithFilters({
      tiempo: 120, dificultad: '3', festividad: '4', predominante: 'rojo', secundario: 'azul'
    });
  ...
  test('getAllWithFilters: solo colorSecundario -> solo $2 y sin otras condiciones', async () => {
    const r = new Repo();
    await r.getAllWithFilters({
      tiempo: 90, dificultad: '0', festividad: '0', predominante: 'todos', secundario: 'azul'
    });
    const { query, params } = CupcakeModel.getAllWithFilters.mock.calls[0][0];
  ...
  test('getByFilters: con todos los filtros -> placeholders $3..$6 y params en orden', async () => {
    const r = new Repo();
    await r.getByFilters({
      email: 'a@a.com', tiempo: 60, dificultad: '2', festividad: '3', predominante: 'rojo', secundario: 'verde'
    });
    ...
    expect(params).toEqual(['a@a.com', 60, '2', '3', 'rojo', 'verde']);
  });
});
