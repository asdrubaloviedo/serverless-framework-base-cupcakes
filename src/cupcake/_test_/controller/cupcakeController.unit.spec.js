...
describe('CupcakeController', () => {
  ...
  test('doTest -> llama servicio', async () => {
    ...
    expect(SC.DoTestCupcake.execute).toHaveBeenCalled();
    expect(r).toBe('ok-test');
  });

  test('getAll pasa params', async () => {
    const p = { email:'a', tiempo:1, dificultad:'2', festividad:'3', predominante:'rojo', secundario:'azul' };
    const r = await C.getAll(p);
    expect(SC.GetAllCupcake.execute).toHaveBeenCalledWith(p);
    expect(r).toEqual(['all']);
  });

  test('getAllNameImage mapea params', async () => {
    const p = { email:'a', estado:2, categoria:3, festividad:4 };
    await C.getAllNameImage(p);
    expect(SC.GetAllNameImageCupcake.execute).toHaveBeenCalledWith(p);
  });

  test('getAllNameImageMovies con email', async () => {
    await C.getAllNameImageMovies({ email:'a@a.com' });
    expect(SC.GetAllNameImageMoviesCupcake.execute).toHaveBeenCalledWith({ email:'a@a.com' });
  });

  test('getById -> retorna filas, y null -> mensaje', async () => {
    let r = await C.getById({ id:7 });
    expect(SC.GetByIdCupcake.execute).toHaveBeenCalledWith({ id:7, email:undefined, tiempo:undefined, dificultad:undefined, festividad:undefined, predominante:undefined, secundario:undefined });
    expect(r).toEqual(['byid']);

    SC.GetByIdCupcake.execute.mockResolvedValueOnce(null);
    r = await C.getById({ id:7 });
    expect(r).toEqual({ message: 'Cupcake not found' });
  });

  test('getByIdInfoImage -> filas o mensaje', async () => {
    let r = await C.getByIdInfoImage({ id:9 });
    expect(SC.GetByIdInfoImageCupcake.execute).toHaveBeenCalledWith({ id:9 });
    expect(r).toEqual(['infoimg']);

    SC.GetByIdInfoImageCupcake.execute.mockResolvedValueOnce(null);
    r = await C.getByIdInfoImage({ id:9 });
    expect(r).toEqual({ message: 'Info and image not found' });
  });

  test('getByIdCupcakeUserState -> filas o mensaje', async () => {
    let r = await C.getByIdCupcakeUserState({ email:'a', id:1 });
    expect(SU.GetByIdCupcakeUserStateCupcake.execute).toHaveBeenCalledWith({ email:'a', id:1 });
    expect(r).toEqual(['states']);

    SU.GetByIdCupcakeUserStateCupcake.execute.mockResolvedValueOnce(null);
    r = await C.getByIdCupcakeUserState({ email:'a', id:1 });
    expect(r).toEqual({ message: 'Cupcake user states were not found' });
  });

  test('createOneCupcakeUserState / patchOneCupcakeUserState', async () => {
    const p = { email:'a', cupcake:1, estado:2 };
    await C.createOneCupcakeUserState(p);
    expect(SU.CreateOneCupcakeUserStateCupcake.execute).toHaveBeenCalledWith(p);

    await C.patchOneCupcakeUserState({ ...p, valor:true });
    expect(SU.PatchOneCupcakeUserStateCupcake.execute).toHaveBeenCalledWith({ ...p, valor:true });
  });

  test('getAllRamdom', async () => {
    const r = await C.getAllRamdom();
    expect(SC.GetAllRamdomCupcake.execute).toHaveBeenCalled();
    expect(r).toEqual(['rand']);
  });

  test('getAllNameImageFiltros mapea params', async () => {
    const p = { tiempo:150, dificultad:'media', festividad:'navidad', colorpredominante:'rojo', colorsecundario:'azul' };
    await C.getAllNameImageFiltros(p);
    expect(SC.GetAllNameImageFiltrosCupcake.execute).toHaveBeenCalledWith(p);
  });
});
